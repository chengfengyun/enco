...
function SOLVE_THE_PAIR(self, opposite)
	print("------------------------------------------------")
	print("    Self:", self.Self_ProjDuration, self.Oppo_ProjDuration)
	print("Opposite:", opposite.Self_ProjDuration, opposite.Oppo_ProjDuration)

	if( self.Self_ProjDuration >= opposite.Oppo_ProjDuration - 0.5 and
		self.Self_ProjDuration <= opposite.Oppo_ProjDuration + 0.5 and
	    opposite.Self_ProjDuration >= self.Oppo_ProjDuration - 0.5 and
		opposite.Self_ProjDuration <= self.Oppo_ProjDuration + 0.5) then
		print('\t*Selected')
		return 1.0
	end
